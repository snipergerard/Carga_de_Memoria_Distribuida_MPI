...
int main(int argc, char* argv[]) {
    ...
    int N = 40;
    int imprimir_detalles = (N == 40) ? 1 : 0;
    ...
    do {
        if (rank_mpi == 0) {
            printf("\n=== MENU PRINCIPAL (SEND/RECV) (Nodo Maestro: %s) ===\n", nombre_equipo);
        ...
        switch(opcion_menu) {
            case 1:
                ...
                if (rank_mpi == 0) {
                    ...
                    for (int origen = 1; origen < num_procesos; origen++) {
                        int offset = origen * size_local;
                        MPI_Recv(&A[offset], size_local, MPI_INT, origen, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        MPI_Recv(&B[offset], size_local, MPI_INT, origen, 11, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    }
                } else {
                    MPI_Send(sub_A, size_local, MPI_INT, 0, 10, MPI_COMM_WORLD);
                    MPI_Send(sub_B, size_local, MPI_INT, 0, 11, MPI_COMM_WORLD);
                }
                break;
            ...
            case 2: case 3: case 4: case 5:
                if (rank_mpi == 0) {
                    for (int i = 0; i < size_local; i++) {
                        sub_A[i] = A[i];
                        if (opcion_menu != 5) sub_B[i] = B[i];
                    }
                    for (int dest = 1; dest < num_procesos; dest++) {
                        int offset = dest * size_local;
                        MPI_Send(&A[offset], size_local, MPI_INT, dest, 20, MPI_COMM_WORLD);
                        if (opcion_menu != 5) MPI_Send(&B[offset], size_local, MPI_INT, dest, 21, MPI_COMM_WORLD);
                    }
                } else {
                    MPI_Recv(sub_A, size_local, MPI_INT, 0, 20, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    if (opcion_menu != 5) MPI_Recv(sub_B, size_local, MPI_INT, 0, 21, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                }
                ...
                if (rank_mpi == 0) {
                    for (int i = 0; i < size_local; i++) C[i] = sub_C[i];
                    for (int origen = 1; origen < num_procesos; origen++) {
                        int offset = origen * size_local;
                        MPI_Recv(&C[offset], size_local, MPI_INT, origen, 22, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    }
                } else {
                    MPI_Send(sub_C, size_local, MPI_INT, 0, 22, MPI_COMM_WORLD);
                }
                break;
        ...
        if (rank_mpi == 0 && opcion_menu >= 1 && opcion_menu <= 5) {
            ...
            printf("---------------------\n");
            fflush(stdout); // PARCHE: Vaciado final de los resultados
        }
    } while (opcion_menu != 0);
